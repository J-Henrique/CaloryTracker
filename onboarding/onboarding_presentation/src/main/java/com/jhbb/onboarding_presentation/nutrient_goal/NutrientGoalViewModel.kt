...
import com.jhbb.onboarding_domain.use_case.ValidateNutrients
    ...
    fun onEvent(event: NutrientGoalEvent) {
        when (event) {
            ...
            is NutrientGoalEvent.OnFatRationEnter -> {
                state = state.copy(fatRatio = filterOutDigits(event.fat))
            }
            is NutrientGoalEvent.OnProteinRationEnter -> {
                state = state.copy(proteinRatio = filterOutDigits(event.protein))
            }
